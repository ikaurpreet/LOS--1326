...
export default class BaseComponent extends LightningElement {
  ...
  /**
   * updateParticipant
   * @param {function} mutationCallback - callback function
   * @param {object} variables - object with new values to be updated
   * @param {Mutations} mutations - query body
   */
  updateParticipant(mutationCallback, variables, mutations) {
    let gqlQuery = null;

    if (!mutations || (!mutations.REFINANCE_SUBMISSION_MUTATION && !mutations.PURCHASE_SUBMISSION_MUTATION)) {
      console.error('No query provided');
      return;
    }
    if (this.submissionType === 'MortgageRefi') {
      gqlQuery = mutations.REFINANCE_SUBMISSION_MUTATION;
    } else if (this.submissionType === 'HomePurchase') {
      gqlQuery = mutations.PURCHASE_SUBMISSION_MUTATION;
    } else {
      mutationCallback({});
      console.error('Submission is not Mortgages Refinance nor Home Purchase.');
      return;
