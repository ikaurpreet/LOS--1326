...
import DOCUMENT_OPPORTUNITY_ID_FIELD from '@salesforce/schema/Document__c.Opportunity__c';
...
export default class MortgagesActionsDocument extends LightningElement {
  ...
  @wire(getRecord, { recordId: '$recordId', fields: [DOCUMENT_NAME_FIELD, DOCUMENT_FILENAME_FIELD, DOCUMENT_BUSINESS_NAME_FIELD, DOCUMENT_OPPORTUNITY_ID_FIELD] })
  _document;
  ...
  get canArchive() {
    return this.document && this.document.status !== 'Archived' && this.document.status !== 'Re-requested';
  }
  ...
  get canRename() {
    return this.objectApiName === 'Document__c' && this.document && !['Archived', 'Uploaded', 'Outstanding'].includes(this.document.status);
  }

  get canCreateCondition() {
    const statusesToShow = ['Outstanding', 'Uploaded', 'Awaiting Approval', 'Accepted', 'Archived'];
    return this.objectApiName === 'Document__c' && 
           this.document && 
           this.document.canCreateCondition && 
           statusesToShow.includes(this.document.status);
  }
  ...
  get opportunityId() {
    if (this.objectApiName === 'Document__c') {
      const opportunityId = getFieldValue(this._document.data, DOCUMENT_OPPORTUNITY_ID_FIELD);
  ...
  createConditionHandler() {
    const map = { 'Income': 'w2Paystub', 'Assets': 'asset', 'Property': 'property', 'Other': 'other', 'Misc': 'other' };
    this.documentCategory = map[this.document.category];
    this.documentParticipantRole = this.document.participantRole;
    this.openAddCondition = !this.openAddCondition;
  }
  ...
  showNotification(message) {
    ...
    if (this.document.opportunity) {
      publish(this.messageContext, OpportunityChangeChannel, { OpportunityId: this.document.opportunity.Id });
    }
  }
