...
export default class MortgagesSubmissionArchivedDocuments extends LightningElement {
  @api recordId;
  @track data = [];
  ...
  openAddLenderDocumentHandler(e) {
    ...
    this.addLenderTags = this.documentsMap[this.addLenderDocumentId].lenderTags.map((item) => item.value);
    this.x = e.target.getBoundingClientRect().x
    this.y = e.target.getBoundingClientRect().y
  }

  updateAddLenderDocumentHandler(e) {
    this.documentsMap[this.addLenderDocumentId].lenderTags = e.detail;
    this.buildFolderLabel(this.documentsMap[this.addLenderDocumentId]);
  ...
  loadData(loading) {
    ...
    this.error = null;
    this.documentsMap = {};
    const promise = fetchDocuments({ opportunityId: this.recordId, onlyArchived: true });
